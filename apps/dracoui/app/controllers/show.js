...
const getAnchoredHeadings = (container) => {
  let headings = [];
  // Defensive check: Ensure container is not null before proceeding
  if (!container) {
    console.warn("Warning: .doc-page-content container not found. Table of Contents will not be generated for this section.");
    return headings; // Return empty array if container is null
  }
  container
...
export default class ShowController extends Controller {
  ...
  didInsertContent = () => {
    ...
    if (documentSections.length > 0) {
      ...
      console.log("querySelector('.doc-page-content') result:", container); // Debug log
      tocs.push({
        index: 0,
        id: 'toc-all',
        // we show only the headings level 2 and 3 in the sidecar
